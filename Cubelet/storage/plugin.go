...
// cubecow (reflink-only copy-on-write) backend.
const StorageBackendCow = "cubecow"

// StorageBackendOverlayFS enables container mode using containerd's native
// overlayfs snapshotter. No XFS/reflink/CGO required.
const StorageBackendOverlayFS = "overlayfs"
...
func init() {
	registry.Register(&plugin.Registration{
		...
		InitFn: func(ic *plugin.InitContext) (interface{}, error) {
			...
			if localStorage.config.StorageBackend == StorageBackendOverlayFS {
				// Container mode: storage handled by containerd overlayfs snapshotter.
				// No cubecow/XFS/reflink needed.
				CubeLog.Infof("storage backend: overlayfs (container mode)")
			} else if localStorage.config.StorageBackend == StorageBackendCow {
